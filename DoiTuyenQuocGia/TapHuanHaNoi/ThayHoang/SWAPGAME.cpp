...
void dfs(int u,int p,int cur_color)
{
    //cout << u << ' ' << cur_color << endl;
    color[u] = cur_color;
    d[cur_color]++;
    for(int i = 0;i < edge[u].size();i++)
    {
        ...
        int w = edge[u][i].se;
        //cout << v << ' ' << w << endl;
        int new_color = (cur_color + w)%2;
        if(v == p) continue;
        if(color[v] == -1) dfs(v,u,new_color);
        else
        {
            if(color[v] != new_color)
            {
                cout << 0 << endl;
                exit(0);
            }
...
int main()
{
    ...
    For(i,1,n)
    {
        ...
        int r1 = (i == a[0][pos1]) ? 0 : 1;
        int r2 = (i == a[0][pos2]) ? 0 : 1;
        if(r1 == r2)
        {
            edge[pos1].push_back({pos2,1});
            edge[pos2].push_back({pos1,1});
           // cout << "1 " << pos1 << ' ' << pos2 << endl;
        }
        else
        {
            edge[pos1].push_back({pos2,0});
            edge[pos2].push_back({pos1,0});
            //cout << "0 " << pos1 << ' ' << pos2 << endl;
        }
    }
    For(i,1,n)
        if(color[i] == -1)
