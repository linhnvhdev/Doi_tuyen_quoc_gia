...
void xuly(int i,bigint j)
{
    if(i==n+1) return;
    bigint tmp=pw[n-i]-F(i,deg+1);
    if(tmp<j)
    {
        kq.push_back(')');
        j-=tmp;
...
main()
{
    ...
    //memset(f,-1,sizeof f);
    cin >> n >> k;
    forinc(i,0,n+3) forinc(j,0,n+3) f[i][j]=-1;
    pw[0]=1;
    forinc(i,1,n+1) pw[i]=pw[i-1]*2;
//    pw[0]=1;
//    forinc(i,1,n+5) pw[i]=pw[i-1]*2;
    xuly(1,k);
    //cout << deg;
    cout << kq;
}
