...
bignum f[N][N][2],ans;
string a;
bignum F(int i,int j,int o)
{
    if(i==n+1) return j;
    if(dd[i][j][o]) return f[i][j][o];
    dd[i][j][o]=1;
    f[i][j][o]=0;
    if(!o) forinc(t,0,a[i]-'0') f[i][j][o]=f[i][j][o]+F(i+1,j+(t==x),(t<a[i]-'0'));
    else forinc(t,0,9) f[i][j][o]=f[i][j][o]+F(i+1,j+(t==x),1);
    return f[i][j][o];
}
int main()
{
