...
pair<int,int> vt[maxn*maxn],P[maxn*maxn];
...
void KhoiTao()
{
    For(i,1,n)
        ...
        {
            int add = (a[i][j] == 4) ? t : 0;
            if(a[i][j] == 5) magic.push_back({i,j});
            if(a[i][j] != 5){
                For(k,0,3)
                {
    ...
    for(int ii = 0;ii < magic.size();ii++)
        for(int jj = 0;jj < magic.size();jj++)
        {
            pii x = magic[ii],y =magic[jj];
