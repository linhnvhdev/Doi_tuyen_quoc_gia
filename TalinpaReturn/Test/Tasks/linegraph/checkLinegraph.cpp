...
ifstream fin("linegraph.inp");
// Open our output
ifstream fok("linegraph.out");
// Open their output
...
void check_test() {
    ...
    if (our_answer == "YES") {
        ...
        Assert((int)t_ours.size() == N_ours - 1, "OK doesn't contain a tree!");
        // Check OK is a tree
    ...
    if (their_answer == "YES") {
        ...
        Assert((int)t_theirs.size() == N_theirs - 1, "Output file doesn't contain a tree!");
